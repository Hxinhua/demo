...
function getecharts() {
  chatheight.value = props.supplierRanklist.length * 34 + 'px'
  setTimeout(() => {
    const myechart = echarts.init(chatMain.value, 'chatMain')
    const yAxisdata = []
    const last30DaysRecycling = []
    const rankNo = []
    for (let index = 0; index < props.supplierRanklist.length; index++) {
      const element = props.supplierRanklist[index];
      yAxisdata.push(element.supplier)
      ...
      series: [{
        ...
        itemStyle: {
          ...
          color: new echarts.graphic.LinearGradient(
            1, 0, 0, 0,
            [
              { offset: 0, color: '#0deee0' },
              { offset: 0.5, color: '#13b2e9' },
              { offset: 1, color: '#2168dc' }
            ]
          )
        },
        emphasis: {
